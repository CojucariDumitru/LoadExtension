...
const hasBuildVersion = content.includes('0.4.0');
...
if (version === "0.4.0" && usesBootstrap && hasBuildVersion && !hasOldToolbar) {
  console.log(`  status           : OK — reload extension in chrome://extensions\n`);
  process.exit(0);
}

console.log(`  status           : OUTDATED — your source files are old`);
console.log(`  expected         : manifest 0.4.0, bootstrap.js, ~23kb content.js`);
